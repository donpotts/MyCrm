...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("MyCrm") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "MyCrm", DisplayName = "MyCrm API", Resources = { "MyCrm" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["MyCrm_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["MyCrm_Web:RootUrl"]!.EnsureEndsWith('/');

            /* MyCrm_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["MyCrm_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["MyCrm_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["MyCrm_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        // App Client
        var appClientId = configurationSection["MyCrm_App:ClientId"];
        if (!appClientId.IsNullOrWhiteSpace())
        {
                await CreateApplicationAsync(
                    ...
                    secret: configurationSection["MyCrm_App:ClientSecret"] ?? "1q2w3e*",
